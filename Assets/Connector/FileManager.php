 ...
 *
 * Options:
 *   - documentRootPath: (string) The URI base directory to be used for the FileManager ('URI path' i.e. an absolute path here would be rooted at DocumentRoot: '/' == DocumentRoot)
 *   - assetPath: (string, optional) The URI path to all images and swf files used by the filemanager
 ...
 *   - mimeTypesFilePath: (string, optional) The filesystem path to the MimeTypes.ini file. May exist in a place outside the DocumentRoot tree.
...
require(strtr(dirname(__FILE__), '\\', '/') . '/KLogger.php');
 ...
 * FileManager-managed directory tree, rooted by options['documentRootPath'], can become a huge collection,
...
//		$this->file = $fm_obj->legal_url_path2file_path($legal_url);
		$this->file = $fm_obj->getAbsolutePath($legal_url);
		$this->fstat = NULL;
		...
		$this->cache_dir_url = $fmopts['thumbnailCacheUrl'] . $dir;
...
class FileManager
{
	...
	protected $managedBaseDir;				// precalculated filesystem path eqv. of options['documentRootPath']
	protected $assetPath;					// Assets absolute filesystem path
	
	protected $mimeTypesFilePath;
	...
	public function __construct($options)
	{
		...
		$this->options = array_merge(array(
			 ...
			 */
			'requestScript' => NULL,			// default is $_SERVER['SCRIPT_NAME']

			'baseUrl' => NULL,					// an absolute filesystem path pointing at URI path '/'. Default: SERVER['DOCUMENT_ROOT']
			'documentRootPath' => NULL,					// an absolute filesystem path pointing at URI path '/'. Default: SERVER['DOCUMENT_ROOT']
			'directory' => NULL,						// Relative to the the DocumentRoot path.			
			'assetPath' => NULL,						// may sit outside options['documentRootPath'] but MUST be in the DocumentRoot tree
			'thumbnailPath' => NULL,					// may sit outside options['documentRootPath'] but MUST be in the DocumentRoot tree
			
			'thumbSmallSize' => 48,						// Used for thumb48 creation
			'thumbBigSize' => 250,						// Used for thumb250 creation
			'mimeTypesFilePath' => strtr(dirname(__FILE__), '\\', '/') . '/MimeTypes.ini',  // an absolute filesystem path anywhere; when relative, it will be assumed to be against options['requestScript']
		...
		// apply default to requestScript:
		if (empty($this->options['requestScript']))
			$this->options['requestScript'] = $this->getRequestURI();

		$this->options['thumbnailPath'] = self::addTrailingSlash($this->options['thumbnailPath']);

		$this->baseUrl = 					$this->options['baseUrl']; 
		
		$this->documentRootPath = 			self::addTrailingSlash($assumed_root);
		
		// Assets
		$this->options['assetPath'] = 		self::addTrailingSlash($this->options['assetPath']);
		...
		$this->managedBaseDir = 			$this->documentRootPath . trim($this->options['directory'], '/') .'/';
		$this->managedBaseUrl = 			$this->baseUrl . trim($this->options['directory'], '/') .'/';

		// By default for the moment
		// This way of config allows to change easily the store folder of thumbs if wished
		$this->thumbnailCacheDir = 			$this->documentRootPath . trim($this->options['thumbnailPath'], '/') .'/';
		$this->thumbnailCacheUrl = 			$this->baseUrl . trim($this->options['thumbnailPath'], '/') .'/';

		$this->thumbnailCacheParentDir = 	$this->documentRootPath . (self::getParentDir($this->options['thumbnailPath']));

		// Mimes types file
		$mimeTypesFilePath = @realpath($this->options['mimeTypesFilePath']);

		if (empty($mimeTypesFilePath))
			throw new FileManagerException('nofile');
		
		$this->mimeTypesFilePath = strtr($mimeTypesFilePath, '\\', '/');
	...
	/**
	 * Returns the FileManager options and settings.
	 * Used by MTFMCacheItem
	 *
	 * @return	Array	Array if settings
	 *
	 */
	public function getSettings()
	{
		return array_merge(array(
			'thumbnailCacheUrl' => $this->thumbnailCacheUrl,
...
//		$dir = $this->legal_url_path2file_path($legal_url);
		...
		$dir = $this->managedBaseDir . $legal_url;
...
/*
$this->log->logInfo('$l_url : ' . $l_url);
$this->log->logInfo('$file : ' . $file);
*/
		...
		return array_merge((is_array($json) ? $json : array()), array(
			'root' => substr($this->documentRootPath, 1),
			'this_dir' => array(
				'path' => $legal_url,
	 ...
	 * Expected parameters:
	 *
	 * $_POST['directory']     path relative to basedir a.k.a. options['documentRootPath'] root
	 *
	 ...
	 *                         must end up inside the options['documentRootPath']-rooted subtree to be
			...
			try
			{
				if ( ! empty($file_preselect_arg))
				{
					...
					if (strpos($file_preselect_arg, '/') !== FALSE)
					{
						...
						$legal_url = self::addTrailingSlash($legal_url);
					}
					else
					{
...
//				$this->sendHttpHeaders('Content-Type: application/json');
	...
	protected function onDetail()
	{
		...
		try
		{
			...
			if ( ! empty($file_arg))
			{
				...
				$rel_path = $this->normalize($this->normalize($this->getPOSTparam('directory')) . '/'. $filename);
				
				// absolute system file path
				$file = $this->getAbsolutePath($rel_path);

				if (is_readable($file))
				{
					if (is_file($file))
					{
						$meta = $this->getFileInfo($file, $rel_path);
			...
			$jserr = $this->extractDetailInfo($jserr, $rel_path, $meta, $mime_filter, $mime_filters, $mode);
	...
	protected function onCreate()
	{
		...
		catch(FileManagerException $e)
		{
			...
			catch (Exception $e)
			{
				...
				try
				{
					$legal_url = $this->documentRootPath;
					$jserr = $this->_onView($legal_url, $jserr, $mime_filter);
				}
				catch (Exception $e)
				{
	 ...
	 * 'basedir' (options['documentRootPath']) will be allowed to be downloaded.
	 *
	 * Expected parameters:
	 *
	 ...
	 *
	 *   $_POST['newDirectory']  path relative to basedir a.k.a. options['documentRootPath'] root;
	...
	public function extractDetailInfo($json_in, $legal_url, &$meta, $mime_filter, $mime_filters, $mode)
	{
		...
		$file = $this->getAbsolutePath($legal_url);
		
		// Absolute URL to the file
		$url = $this->getAbsoluteUrl($legal_url);
...
// $this->log->logInfo('$thumb250 :::' . $thumb250);
				...
				if (empty($thumb250))
				{
					if ($thumbnails_done_or_deferred)
					{
						...
						$preview_HTML = '<a href="' . FileManagerUtility::rawurlencode_path($url) . '" data-milkbox="single" title="' . htmlentities($filename, ENT_QUOTES, 'UTF-8') . '">
									   <img src="' . $this->assetPath . 'Images/transparent.gif" class="preview" alt="preview" style="width: ' . $dims['width'] . 'px; height: ' . $dims['height'] . 'px;" />
									 </a>';
...
// $this->log->logInfo('$thumb250 : ' . $this->url_path2file_path($thumb250));
// $meta->store('mime_type from file extension', $mime2);
...
// $this->log->logInfo(print_r($meta, true));
			
//				$tnsize = getimagesize($this->url_path2file_path($thumb250));
				$thumbPath = $meta->getThumbPath('250x250');

//				$tnsize = getimagesize($thumb250);
				$tnsize = getimagesize($thumbPath);

 $this->log->logInfo(print_r($tnsize, true));


// $this->log->logInfo('$tnsize : '  .$this->url_path2file_path($thumb250), true);
	...
	protected function unlink($legal_url, $mime_filters)
	{
		...
		if (is_dir($file))
		{
			$dir = self::addTrailingSlash($file);
			$url = self::addTrailingSlash($legal_url);
	 ...
	 * directory tree rooted by options['documentRootPath']
	 *
	...
	public function getIcon($file, $smallIcon)
	{
		...
		$icon_path = $this->assetPath . 'Images/Icons/' . $largeDir . $ext . '.png';
		$icon_url = $this->options['assetPath'] . 'Images/Icons/' . $largeDir . $ext . '.png';

		
		$url = (is_file($icon_path))
			? $icon_url
			: $this->options['assetPath'] . 'Images/Icons/' . $largeDir . 'default.png';

/*
 $this->log->logInfo('$icon_path : ' . $icon_path);
 $this->log->logInfo('ICON url : ' . $url);
*/


		$this->icon_cache[!$smallIcon][$ext] = $url;
	 ...
	 *
	 * This default can be overridden by specifying the options['requestScript'] when invoking the constructor.
	 */

	public function getRequestURI()
	{
		return strtr($_SERVER['SCRIPT_NAME'], '\\', '/');
	
/*
		if ( ! empty($this->options['requestScript']))
		{
			return $this->options['requestScript'];
		}
...
*/
	...
	{
		$path = self::getParentDir($this->getRequestURI());
...
//echo('path : ' . $path);
	...
	/**
	 * Accept an absolute URI path, i.e. rooted against DocumentRoot, and transform it to a LEGAL URI absolute path, i.e. rooted against options['documentRootPath'].
	 *
	 * Relative paths are assumed to be relative to the current request path, i.e. the getRequestPath() produced path.
	 *
	 ...
	 *
	 * Throws a FileManagerException when the given path cannot be converted to a LEGAL URL, i.e. when it resides outside the options['documentRootPath'] subtree.
	 */
	public function abs2legal_url_path($path)
	{
		...
		// but we MUST make sure the path is still a LEGAL URI, i.e. sitting inside options['documentRootPath']:
	 ...
	 *
	 * Relative paths are assumed to be relative to the options['documentRootPath'] directory. This makes them equivalent to absolute paths within
	...
	/**
	 * Accept a relative or absolute LEGAL URI path and transform it to an absolute LEGAL URI path, i.e. rooted against options['documentRootPath'].
	 *
	 * Relative paths are assumed to be relative to the options['documentRootPath'] directory. This makes them equivalent to absolute paths within
	 * the LEGAL URI tree and this fact may seem odd. Alas, the FM frontend sends requests without the leading slash and it's those that
	 * we wish to resolve here, after all. So, yes, this deviates from the general principle applied elesewhere in the code. :-(
...
//log_message('error', 'FileSystemPath4SiteDocumentRoot :' . $this->options['FileSystemPath4SiteDocumentRoot']);
		$path = $this->documentRootPath . $url_path;
	...
	{
		$path = $this->normalize($path);
		return rtrim($this->managedBaseDir, '/') . $path;
	}

	public function getAbsoluteUrl($path)
	{
		$path = $this->normalize($path);
		return rtrim($this->managedBaseUrl, '/') . $path;
	}
	...
	public function getMimeTypeDefinitions()
	{
		...
		if (!$mimes)
		{
			$mimes = parse_ini_file($this->mimeTypesFilePath);
...
//			error_reporting($ini);
	 ...
	 *
	 * @param string $file       physical filesystem path to the file we want to get info
	 *
	...
	protected function getGETparam($name, $default_value = NULL)
	{
	...
	protected function getPOSTparam($name, $default_value = NULL)
	{
